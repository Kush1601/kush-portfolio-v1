  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particles.forEach((p) => {
        ...
        // Riso misregistration on dark: coral channel printed slightly off,
        // cobalt channel on top — additive (screen) so the dots glow.
        const a = p.currentAlpha;
        ctx.fillStyle = `rgba(255, 111, 82, ${a * 0.9})`;
        ctx.fillText(p.char, p.x + 1.6, p.y + 1.3);
        ctx.fillStyle = `rgba(104, 119, 255, ${a})`;
